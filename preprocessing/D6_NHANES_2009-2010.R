...
# Objective           : Reading and cleaning NHANES 2009-2010
# Owner               : Jithin
...
#
# Modules :
#     1. Reading and cleaning NHANES 2009-2010 data for Whites
#----------------------------------------------------------------------------
...
# 1. Reading and cleaning NHANES 2009-2010 data ----------------

path_data <- "D:/St John's National Academy of Health Sciences/Data Science - 22_Evaluating Hb cutoffs/data"

cbc <- sasxport.get(paste0(path_data,"/nhanes/CBC_F.XPT"),allow=FALSE,as.is=TRUE)
demo <- sasxport.get(paste0(path_data,"/nhanes/DEMO_F.XPT"),allow=FALSE,as.is=TRUE)
hsq <- sasxport.get(paste0(path_data,"/nhanes/HSQ_F.XPT"),allow=FALSE,as.is=TRUE)
huq <- sasxport.get(paste0(path_data,"/nhanes/HUQ_F.XPT"),allow=FALSE,as.is=TRUE)
fertin <- sasxport.get(paste0(path_data,"/nhanes/FERTIN_F.XPT"),allow=FALSE,as.is=TRUE)
tfr <- sasxport.get(paste0(path_data,"/nhanes/TFR_F.XPT"),allow=FALSE,as.is=TRUE)
biopro <- sasxport.get(paste0(path_data,"/nhanes/BIOPRO_F.XPT"),allow=FALSE,as.is=TRUE)
crp <- sasxport.get(paste0(path_data,"/nhanes/CRP_F.XPT"),allow=FALSE,as.is=TRUE)
folate <- sasxport.get(paste0(path_data,"/nhanes/FOLATE_F.XPT"),allow=FALSE,as.is=TRUE)
...
col_nhanes <- c("seqn","aim","gender","pregnancy_status","who_cat",
                "hb","health_status",
                "iron_ug","iron_umol","transferrin_receptor_mg_per_l",
...
write.xlsx(as.data.frame(df),
           ...
           sheetName = "2009-2010",append=TRUE,row.names=FALSE)

saveRDS(df,file=paste0(path_data , "/working/nhanes_2009to2010_v2.RDS"))
