...
# Objective           : Pooled analysis of Hb cutoffs for South Asians
# Owner               : Jithin
...
nhanes_2013 <- readRDS(paste0(path_data,"/working/nhanes2013-14_non_hisp_asian_np.RDS"))
ndns_y1to4 <- readRDS(paste0(path_data,"/working/ndns_y1to4_individual_sa_f_a20u45_hb.RDS"))
ndns_19to64 <- readRDS(paste0(path_data,"/working/ndns_19to64_bloodanalytes_sa_f_a20u45_hb_np.RDS"))
...
write.xlsx(as.data.frame(consolidated),
           ...
           sheetName = "consolidated",append=TRUE,row.names=FALSE)

write.xlsx(as.data.frame(ndns_19to64),
           file=paste0(path_data , "/working/evaluating_cutoffs_",Sys.Date(),".xlsx"),
           sheetName = "ndns_19to64",append=TRUE,row.names=FALSE)

write.xlsx(as.data.frame(ndns_y1to4),
           file=paste0(path_data , "/working/evaluating_cutoffs_",Sys.Date(),".xlsx"),
           sheetName = "ndns_y1to4",append=TRUE,row.names=FALSE)

write.xlsx(as.data.frame(nhanes_2013),
           file=paste0(path_data , "/working/evaluating_cutoffs_",Sys.Date(),".xlsx"),
           sheetName = "nhanes_2013",append=TRUE,row.names=FALSE)

# 2. Pooled histogram -----------------------------------------------------

hist1 <- ggplot() + theme_bw()+ xlab("Haemoglobin") +
  ggtitle("Pooled distribution for non-pregnant women 15 to 49 (N=215)")
